...
function styles() { // таск 'styles' обработает все файлы *.sass, вложенные в любые подпапки
	...
	.pipe(sass({ outputStyle: 'expanded' }).on('error', sass.logError)) //  Опция { outputStyle: 'expanded' } развертывает все унификации
	.pipe(autoprefixer({
		grid: true,
...
function deploy() {
	...
	.pipe(rsync({
		root: path.static,
		hostname: 'username@cool-int.ru',
		destination: 'cool-int.ru/public_html/',
